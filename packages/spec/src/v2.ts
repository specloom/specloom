...
export interface CompiledSpecV2 {
  version: "2";
...
export type SpecloomSpecV2 = CompiledSpecV2;
...
export class SpecV2Error extends Error {
  constructor(
    message: string,
    readonly path?: string,
  ) {
    super(path ? `${message} at ${path}` : message);
    this.name = "SpecV2Error";
  }
}

export function parseSpecV2(source: string): CompiledSpecV2 {
  let parsed: unknown;
  try {
    parsed = JSON.parse(source) as unknown;
  } catch (error) {
    throw new SpecV2Error(
      error instanceof Error ? error.message : "Invalid JSON",
      "$",
    );
  }

  return validateSpecV2(parsed);
}

export function isSpecV2(value: unknown): value is CompiledSpecV2 {
  try {
    validateSpecV2(value);
    return true;
  } catch {
    return false;
  }
...
export function validateSpecV2(value: unknown): CompiledSpecV2 {
  const root = expectRecord(value, "$");

  if (root.version !== "2") {
    throw new SpecV2Error(`Expected version "2"`, "$.version");
  }
...
export function normalizeSpecV2(spec: CompiledSpecV2): CompiledSpecV2 {
  const validated = validateSpecV2(spec);
...
function validateFieldType(value: unknown, path: string): CompiledFieldType {
  ...
  switch (kind) {
    ...
    default:
      throw new SpecV2Error(`Unsupported field type kind "${kind}"`, path);
  }
}
...
function validateFilterValue(value: unknown, path: string): FilterValue {
  ...
  if (Array.isArray(value)) {
    ...
    throw new SpecV2Error("Unsupported array filter value", path);
  }
  ...
  throw new SpecV2Error("Unsupported filter value", path);
}
...
function validateExpressionAst(
  ...
  switch (type) {
    case "literal":
      ...
      throw new SpecV2Error("Invalid literal value", `${path}.value`);
    case "identifier":
    ...
    default:
      throw new SpecV2Error(`Unsupported expression type "${type}"`, path);
  }
}
...
function expectRecord(value: unknown, path: string): Record<string, unknown> {
  if (!isPlainRecord(value)) {
    throw new SpecV2Error("Expected object", path);
  }
  return value;
}
...
function expectArray(value: unknown, path: string): unknown[] {
  if (!Array.isArray(value)) {
    throw new SpecV2Error("Expected array", path);
  }
  return value;
}
...
function expectString(value: unknown, path: string): string {
  if (typeof value !== "string") {
    throw new SpecV2Error("Expected string", path);
  }
  return value;
}
...
function expectNumber(value: unknown, path: string): number {
  if (typeof value !== "number" || Number.isNaN(value)) {
    throw new SpecV2Error("Expected number", path);
  }
  return value;
}
...
function expectBoolean(value: unknown, path: string): boolean {
  if (typeof value !== "boolean") {
    throw new SpecV2Error("Expected boolean", path);
  }
  return value;
}
...
function expectPrimitiveOptionValue(
  ...
  throw new SpecV2Error("Expected string, number, or boolean", path);
}
