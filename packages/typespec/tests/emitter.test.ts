...
model Profile {
  ...
  age?: int32;
    ...
    const profile = readJson(resolve(outputDir, "profile.json"));

    expect(profile.resources.Profile.fields.age.validation).toMatchObject({
      minValue: 18,
      maxValue: 65,
    });
    expect(profile.resources.Profile.rules[0]).toMatchObject({
      kind: "requireOneOf",
      fields: ["email", "phone"],
    });
    expect(profile.resources.Profile.rules[1]).toMatchObject({
      kind: "requiredIf",
      field: "reason",
    });
  });
  ...
  it("emits a single aggregate file when output-file is provided", () => {
    ...
    compileFixture(outputDir, [
      "--option",
      "@specloom/typespec.output-file=admin/spec.json",
    ]);
...
function createTempDir(): string {
  const dir = mkdtempSync(resolve(tmpdir(), "typespec-test-"));
  tempDirs.push(dir);
  return dir;
}

function createPackageTempDir(): string {
  const dir = mkdtempSync(resolve(packageRoot, ".typespec-test-"));
  tempDirs.push(dir);
  return dir;
}

function compileFixture(outputDir: string, extraArgs: string[] = []): void {
  compileEntry(fixturePath, outputDir, extraArgs);
}
...
function compileSource(
  ...
  const sourceDir = createPackageTempDir();
  const sourcePath = resolve(sourceDir, "main.tsp");
  writeFileSync(sourcePath, source.trimStart(), "utf8");
  compileEntry(sourcePath, outputDir, extraArgs);
}
