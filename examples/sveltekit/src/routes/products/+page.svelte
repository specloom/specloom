<script lang="ts">
  ...
  import { productsListVM, productsShowVM, productsFormVM } from "$lib/data/products";

  type View = "list" | "show" | "form";

  let currentView = $state<View>("list");
  let listVM = $state<ListViewModel>(productsListVM);
  let showVM = $state<ShowViewModel>(productsShowVM);
  let formVM = $state<FormViewModel>(productsFormVM);
